    ...
    return new Promise((resolve, reject) => {
      this.app.outputmanager.getOutputPlugin('adapt', async (error, adapt) => {
        ...
        if(this.metadata.status.error.length) {
          console.error('Some courses failed to export', this.metadata.status.error.map(e => `${e.course._id} ${e.course.title}, ${e}`));
        }
        resolve();
      });
    });
  ...
  exportCourse(course) {
    return new Promise((resolve, reject) => {
      this.adapt.export(course._id.toString(), undefined, undefined, async (error) => {
        ...
        course.path = path.join(this.metadata.path, `${course._id}.zip`);
