const fs = require('fs/promises');
const { MongoClient, ObjectId } = require('mongodb');
const path = require('path');
const prompts = require('prompts');
const Utils = require('./Utils.cjs');

let LIMIT;
let IS_DEBUG;

class Exporter {
  constructor(options) {
    IS_DEBUG = options.IS_DEBUG;
    LIMIT = options.LIMIT;
    this.shouldExportCourses = options.exportCourses !== false;
    this.sourcePath = options.sourcePath;
    this.exportPath = path.join(this.sourcePath, 'at-migrate');
    this.exportData = {}
  }
  async init() {
    this.config = await Utils.loadJson(path.join(this.sourcePath, 'conf', 'config.json'));
    try {
      const exports = await fs.readdir(this.exportPath);
      if(exports.length) {
        const incompleteExports = exports.filter(e => {
          try {
            const meta = Utils.loadJsonSync(path.join(this.exportPath, e, 'export.json'));
            return meta.status?.success?.length !== meta.courses?.length
          } catch {
            return true
          }
        });
        const { action } = await prompts({
          type: 'select',
          name: 'exportPath',
          message: `There are incomplete exports, what would you like to do?`,
          choices: [
            { title: 'Continue existing', value: 'continue' },
            { title: 'Restart', value: 'restart' }
          ]
        });
        if(action === 'continue') {
          const { exportId } = await prompts({
            type: 'select',
            name: 'exportId',
            message: `Choose an export to continue`,
            choices: incompleteExports.map(f => Object.create({ title: f, value: f }))
          });
          this.exportPath = path.join(this.exportPath, exportId);
          this.metadata = await Utils.loadJson(path.join(this.exportPath, 'export.json'));
        }
      } else {
        console.log('\nNo existing exports found, starting afresh.\n');
        throw new Error('No existing exports')
      }
    } catch(e) {
      this.exportPath = path.join(this.exportPath, Date.now().toString())
    }
    if(!this.metadata) {
      const { forceRebuild } = await prompts([{
        type: 'toggle',
        name: 'forceRebuild',
        message: `Do you want to run a full rebuild of each course before exporting?`,
        initial: false
      }]);
      this.metadata = {
        forceRebuild,
        status: { success: [], error: [] }
      }
    }
    const dbclient = new MongoClient(this.config.dbConnectionUri);
    await dbclient.connect();
    this.db = dbclient.db();

    this.tenants = await this.db.collection('tenants').find().toArray();
  }
  
  async run() {
    // reset rather than append: everything but the courses is re-exported on a continued run
    this.metadata.warnings = [];
    await fs.mkdir(path.join(this.exportPath, 'heroes'), { recursive: true });
    await this.startApp();
    
    await this.exportRoles();
    await this.exportUsers();
    await this.exportThemePresets();
    await this.exportCourses();
    
    await this.stopApp();
    await this.saveMetadata();
    return this.metadata;
  }
  
  warn(message) {
    this.metadata.warnings.push(message);
    console.warn(` ! ${message}`);
  }
  
  startApp() {
    console.log('Starting legacy app');
    this.app = require(path.resolve(this.sourcePath, 'lib/application'))();
    // the update check reads the framework's package.json and dies on a missing version, and the
    // export has no use for it either way
    this.app.run({ skipDependencyCheck: true, skipVersionCheck: true });
    this.app.logger.level('console', 'error');
    return new Promise(resolve => {
      this.app.on('serverStarted', async () => {
        console.log(' - App started');
        this.app.usermanager.getCurrentUser = this.getSuperUser.bind(this);
        resolve();
      });
    });
  }
  
  stopApp() {
    return new Promise((resolve, reject) => {
      this.app._httpServer.close();
      this.app._httpServer.once('close', async () => resolve());
    });
  }
  
  async exportRoles() {
    this.metadata.roles = (await this.db.collection('roles').find().toArray()).map(r => {
      return { _id: r._id, name: r.name }
    });
    console.log(`Exported ${this.metadata.roles.length} roles`);
  }
  
  async exportUsers() {
    const roleNameById = new Map(this.metadata.roles.map(r => [r._id.toString(), r.name]));
    const unknownRoles = new Map();
    const roleless = [];
    this.metadata.users = [];
    for(const u of await this.db.collection('users').find().toArray()) {
      // the importer recreates users by email, so one without it can't be migrated at all
      if(!u.email) {
        this.warn(`User ${u._id} has no email address, omitting from the export`);
        continue;
      }
      const roleNames = this.getUserRoleNames(u, roleNameById, unknownRoles);
      if(!roleNames.length) roleless.push(u.email);
      this.metadata.users.push({ 
        _id: u._id,
        email: u.email,
        firstName: u.firstName,
        lastName: u.lastName,
        roleNames,
        _tenantId: u._tenantId
      });
    }
    for(const [roleId, emails] of unknownRoles) {
      this.warn(`Role ${roleId} doesn't exist, dropped from ${emails.length} user(s): ${summarise(emails)}`);
    }
    if(roleless.length) {
      this.warn(`${roleless.length} user(s) have no valid roles and will be imported with none: ${summarise(roleless)}`);
    }
    console.log(`Exported ${this.metadata.users.length} users`);
  }
  
  // legacy installs can carry users pointing at a role that's since been deleted; dropping the
  // reference keeps the export alive where dereferencing it would take the whole run down
  getUserRoleNames(user, roleNameById, unknownRoles) {
    const roleNames = [];
    for(const roleId of user.roles ?? []) {
      const name = roleId && roleNameById.get(roleId.toString());
      if(name) {
        roleNames.push(name);
        continue;
      }
      const key = String(roleId);
      if(!unknownRoles.has(key)) unknownRoles.set(key, []);
      unknownRoles.get(key).push(user.email);
    }
    return roleNames;
  }
  
  async exportThemePresets() {
    this.metadata.themePresets = (await this.db.collection('themepresets').find().toArray()).map(p => {
      return {
        _id: p._id.toString(),
        displayName: p.displayName,
        parentTheme: p.parentTheme,
        properties: p.properties
      };
    });
    console.log(`Exported ${this.metadata.themePresets.length} theme presets`);
  }

  async exportCourses() {
    // a course's theme preset is linked via the config collection (config._themePreset, keyed by _courseId)
    const configs = await this.db.collection('configs').find().toArray();
    // tags are stored as references, and the tags collection isn't exported, so
    // the ids would arrive meaningless — resolve them to titles here, as roles are
    const tagTitleById = new Map((await this.db.collection('tags').find().toArray())
      .map(t => [t._id.toString(), t.title])
      .filter(([, title]) => title));
    const presetByCourseId = new Map(configs.map(cfg => [cfg._courseId?.toString(), cfg._themePreset?.toString()]));
    this.metadata.courses = (await this.db.collection('courses').find().toArray()).map(c => {
      return {
        _id: c._id,
        title: c.title,
        heroImage: c.heroImage || undefined,
        _isShared: c._isShared || undefined,
        _shareWithUsers: (c._shareWithUsers !== undefined && c._shareWithUsers.length > 0) ? c._shareWithUsers : undefined,
        createdBy: c.createdBy,
        tags: resolveTags(c.tags, tagTitleById),
        themePreset: presetByCourseId.get(c._id.toString()) || undefined
      }
    });
    if(!this.shouldExportCourses) {
      return console.log(`Exporting of course src is disabled, only metadata will be exported`);
    }
    console.log(`Attempting to export ${this.metadata.courses.length} courses`);

    return new Promise((resolve, reject) => {
      this.app.outputmanager.getOutputPlugin('adapt', async (error, adapt) => {
        if(error) {
          return reject(error);
        }
        this.adapt = adapt;
        let done = 0;
        for (let i = 0; i < this.metadata.courses.length; i++) {
          const course = this.metadata.courses[i];
          if(LIMIT !== undefined && done === LIMIT) break;
          // success may contain ObjectIds (this run) or strings (reloaded from export.json), so compare as strings
          if(this.metadata.status.success.some(id => id.toString() === course._id.toString())) continue;
          console.log(`Exporting ${course._id} ${course.title}`);
          try {
            if(course.heroImage) {
              const [assetData] = await this.db.collection('assets').find({ _id: new ObjectId(course.heroImage) }).toArray();
              await fs.cp(path.join(this.sourcePath, 'data', this.app.configuration.getConfig('masterTenantName'), assetData.path.slice(1)), path.join(this.exportPath, 'heroes', assetData.filename));
              course.heroImage = 'heroes/' + assetData.filename;
            }
            await this.exportCourse(course);
            this.metadata.status.success.push(course._id.toString());
            console.log(`  Exported ${course._id} ${course.title}`);

          } catch(e) {
            e._courseId = course._id;
            this.metadata.status.error.push(e);
          }
          done++;
        }
        console.log(`${this.metadata.status.success.length} courses exported successfully`);
        if(this.metadata.status.error.length) {
          console.error('Some courses failed to export', this.metadata.status.error.map(e => `${e._courseId}, ${e}`));
        }
        resolve();
      });
    });
  }
  
  exportCourse(course) {
    return new Promise((resolve, reject) => {
      this.adapt.export(course._id.toString(), undefined, undefined, async (error) => {
        if(error) {
          return reject(error);
        }
        const owner = course.createdBy && this.metadata.users.find(u => u._id.toString() === course.createdBy.toString());
        if(!owner) {
          this.warn(`Course '${course.title}' has an unknown owner (${course.createdBy}), it will be imported unowned`);
        }
        course.createdBy = owner ? owner.email : undefined;
        course.path = `${course._id}.zip`;
        await fs.rename(path.join(this.sourcePath, 'temp', this.config.masterTenantID, 'exports', `${this.app.usermanager.getCurrentUser()._id}.zip`), path.join(this.exportPath, course.path));

        resolve(course);
      }, { forceRebuild: this.metadata.forceRebuild });
    });
  }
  
  saveMetadata() {
    return Utils.writeJson(path.join(this.exportPath, 'export.json'), this.metadata);
  }
  
  getSuperUser() {
    if(!this.superUser) {
      this.superUser = this.metadata.users.find(u => u.roleNames.includes('Super Admin'));
      
      if(!this.superUser) {
        throw new Error('Could not determine super user');
      }
      this.superUser.tenant = this.tenants.find(t => t. _id.toString() === this.superUser._tenantId.toString());
      this.superUser = JSON.parse(JSON.stringify(this.superUser));
    }
    return this.superUser;
  }
  
  async cleanUp() {
    // @todo
  }
}

/**
 * A course's tags as titles. A reference that no longer resolves is dropped
 * rather than carried as an id the target install could never interpret.
 */
function resolveTags(tags, tagTitleById) {
  const titles = (tags ?? []).map(id => tagTitleById.get(id?.toString())).filter(Boolean);
  return titles.length ? titles : undefined;
}

function summarise(items, max = 10) {
  return `${items.slice(0, max).join(', ')}${items.length > max ? ` (+${items.length - max} more)` : ''}`;
}

function debug(...args) {
  if(IS_DEBUG) console.log('DEBUG::', ...args);
}

module.exports = Exporter;