...
class Exporter {
  ...
  async init() {
    ...
    this.metadata = {
      path: path.join(this.sourcePath, 'at-migrate', Date.now().toString()),
      forceRebuild,
  ...
  async run() {
    await fs.mkdir(this.metadata.path, { recursive: true });
    await this.startApp();
  ...
  exportCourse(course) {
    return new Promise((resolve, reject) => {
      this.adapt.export(course._id.toString(), undefined, undefined, async (error) => {
        ...
        await fs.rename(path.join(this.sourcePath, 'temp', this.config.masterTenantID, 'exports', `${this.app.usermanager.getCurrentUser()._id}.zip`), path.join(this.metadata.path, course.path));
  ...
  saveMetadata() {
    return Utils.writeJson(path.join(this.metadata.path, 'export.json'), this.metadata);
  }
