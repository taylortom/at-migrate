...
export default class Importer {
  ...
  async importCourse(course) {
    const zipPath = path.resolve(this.importData.exportPath, course.path);
    ...
    if(course._shareWithUsers?.length > 0 || course._isShared === true || course.heroImage) {
      ...
      if(course.heroImage) {
        ...
        course.heroImage = relHeroDest
      }
      // rewrite course.json with extra export data
      fs.writeFileSync(path.join(unzipPath, 'src', 'course', 'en', 'course.json'), JSON.stringify(course, null, 2))
    }
