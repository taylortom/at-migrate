...
export default class Importer {
  ...
  async init() {
    ...
    try {
      ...
      const completed = this.importData.status.success.length + this.importData.status.skip.length;
  ...
  async importCourses() {
    const { data: existingCourses } = await Utils.request('content/query', { 
      ...this.importData, 
      method: 'POST', 
      body: JSON.stringify({ _type: 'course' }),
      headers: { 'Content-Type': 'application/json' }
    });
    const existingTitles = Array.from(new Set(existingCourses.map(c => c.title)));
