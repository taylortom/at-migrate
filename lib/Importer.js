...
export default class Importer {
  ...
  async init() {
    ...
    try {
      ...
      const completed = this.importData.status.success.length + this.importData.status.error.length;
      ...
      if(action === 'Restart') {
        ...
        this.importData.status = { success: [], error: [], skip: [] };
      }
      if(action === 'Exit') {
  ...
  async importCourses() {
    ...
    for (const courseId of this.exportData.status.success) {
      ...
      try {
        ...
        if(this.importData.status.success.includes(course._id)) {
    ...
    Object.entries(this.status).forEach(([key, value]) => {
      this.importData.status[key] = [...new Set([...value, ...this.importData.status[key]])];
    });
    console.log('');
  }
