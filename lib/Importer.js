...
export default class Importer {
  ...
  async init() {
    ...
    try {
      ...
      const courses = (await Utils.request('content/query', { 
        ...this.importData, 
        method: 'POST', 
  ...
  async importCourses() {
    ...
    for (const course of this.exportData.courses) {
      ...
      const progressStr = `${done.toString().padStart(toImportTotal.toString().length, '0')}/${toImportTotal}`;
      try {
        if(!this.unImportedCourseTitles.includes(course.title)) {
          this.status.skip.push(course._id);
          console.log(`  - Course '${course.title}' (${course._id}) found, skipping`);
          continue;
        }
        ...
        console.log(`    ✔  ${progressStr} Import successful`);
      } catch(e) {
        console.log(`    ✘ ${progressStr} Import failed, ${e.message}`);
        debug(e.stack);
...
- Importing 62f078de8752970188748d0e NDCT-BeYouPL-D05-M03-T01-EL
62f078de8752970188748d0e
/var/www/vhosts/beyondblue/export/1750721483370/62f079268752970188748d1a
/var/www/vhosts/beyondblue/export/1750721483370/62f078de8752970188748d0e/src/course/en/assets/62f079268752970188748d1a
