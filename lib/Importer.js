...
export default class Importer {
  ...
  async run() {
    try {
      ...
      try {
        ...
        log('fail', 0, `Theme preset import failed, courses will still be imported: ${e.message}`);
      }
      await this.saveMetadata();
  ...
  async importCourse(course) {
    ...
    if(course._shareWithUsers?.length > 0 || course._isShared === true || course.heroImage || themeVariables) {
    ...
    const res = await Utils.request('adapt/import', {
      ...this.importData,
      method: 'POST',
      body: JSON.stringify({ importPath: unzipPath }),
      headers: { 'Content-Type': 'application/json' }
    });
