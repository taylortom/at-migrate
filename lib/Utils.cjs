...
class Utils {
  ...
  static async requestAll(endpoint, options = {}) {
    ...
    while(true) {
      ...
      // stop when we've fetched the last page; a missing/invalid header (NaN) also stops us rather than looping forever
      const pageTotal = Number(res.headers.get('x-adapt-pagetotal'));
  ...
  static async request(endpoint, options = {}) {
    if(!options.apiUrl) {
      throw new Error(`No API URL set, cannot request '${endpoint}'`);
    }
